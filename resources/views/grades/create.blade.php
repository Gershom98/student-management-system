        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <p class="text-xs sm:text-sm text-slate-500 dark:text-zinc-400 mt-1">
                    Record marks and assign a grade for a student.
                </p>
            </div>
                ...
                <form action="{{ route('grades.store') }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" min="0" max="100" name="score" id="score"
                            value="{{ old('score') }}" placeholder="e.g. 85.5" required
                            class="mt-1 block w-full rounded-xl border-slate-300 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                        @error('score')
                        <span class="text-rose-500 text-xs mt-1 block">{{ $message }}</span>
                        @enderror
                    ...
                    <div>
                        <label for="grade" class="block text-sm font-medium text-slate-700 dark:text-zinc-300">Grade
                            Letter (Optional - Auto Calculated if blank)</label>
                        <input type="text" name="grade" id="grade" value="{{ old('grade') }}"
                            placeholder="e.g. A, B, C, D, F" maxlength="5"
                            class="mt-1 block w-full rounded-xl border-slate-300 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                        @error('grade')
                        <span class="text-rose-500 text-xs mt-1 block">{{ $message }}</span>
                        @enderror
                    ...
                    <!-- Remarks -->
                    <div>
                        <label for="remarks" class="block text-sm font-medium text-slate-700 dark:text-zinc-300">Remarks
                            (Optional)</label>
                        <input type="text" name="remarks" id="remarks" value="{{ old('remarks') }}"
                            placeholder="e.g. Excellent, Pass, Fail"
                            class="mt-1 block w-full rounded-xl border-slate-300 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                        @error('remarks')
                        <span class="text-rose-500 text-xs mt-1 block">{{ $message }}</span>
                        @enderror
